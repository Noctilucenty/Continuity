 ...
 * Version/tag guard.
 *
 * Ensures package.json's version matches the release tag before publishing, so a
 * release can never ship a mismatched version. Used by the release workflow:
 *
 *   node scripts/check-version-tag.js "$GITHUB_REF_NAME"
 *
 * The tag may be passed as the first CLI arg, or read from GITHUB_REF_NAME.
 * A leading "v" on the tag is stripped (v0.11.0 -> 0.11.0).
 */
